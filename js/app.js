(() => {
  ...
  function cacheElements() {
    [
      "requestForm", "category", "productSearch", "productResults", "quantity",
      "unit", "addProductBtn", "productsTableBody", "productsTableWrapper",
      "emptyProducts", "productError", "notes", "notesCount", "confirmation",
      "submitStatus", "submitBtn", "successTemplate"
    ].forEach((id) => {
      elements[id] = $(id);
    });
  }
  ...
  function renderSearchResults() {
    ...
    if (!matches.length) {
      ...
      matches.forEach((product) => {
        ...
        metadata.textContent = `${product.id} · ${product.unit || "SIN UNIDAD"}`;

        button.append(name, document.createElement("br"), metadata);
  ...
  function addProduct() {
    ...
    if (!unit) return showProductError("Capture la unidad del material.");
  ...
  async function submitRequest(event) {
    ...
    const firstInvalidField = validateMainFields();
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || result.mensaje || "El servidor rechazó la solicitud.");
      }
  ...
  function setLoading(active) {
    ...
    elements.submitBtn.querySelector(".submit-label").textContent = active
      ? "Enviando..."
      : "Enviar solicitud";
  }
  ...
  function disableForm() {
    elements.requestForm.querySelectorAll("input, select, textarea, button").forEach((control) => {
